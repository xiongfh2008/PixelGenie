...
/**
 * Replicate API - 支持多种图像编辑模型
 * 免费额度：每月 $5 免费额度
 * 模型：LaMa (去水印)、Stable Diffusion Inpainting
 */
export const replicateImageEdit = async (imageBase64, prompt, apiKey) => {
  try {
    // 使用 LaMa 模型进行图像修复（去水印）
    const response = await fetch('https://api.replicate.com/v1/predictions', {
      ...
      body: JSON.stringify({
        version: 'e35c0e4e8a8c6f0e3c0b0f0e3c0b0f0e3c0b0f0e3c0b0f0e3c0b0f0e3c0b0f0e', // LaMa 模型
        input: {
...
/**
 * HuggingFace Inference API - 使用真正的图像编辑模型
 * 免费额度：有限制但可用
 * 模型：Stable Diffusion Inpainting, LaMa
 */
...
export const selectImageEditingApi = (apiKeys) => {
  ...
  // 优先级：ClipDrop > Remove.bg > Replicate > Stability > HuggingFace
...
export const editImageWithBestApi = async (imageBase64, prompt, apiKeys, maskBase64 = null) => {
  ...
  console.log(`🎨 Using ${provider} for image editing`);
  ...
  try {
    ...
    if (remainingProviders.length > 0) {
      console.log(`🔄 Trying fallback provider: ${remainingProviders[0]}`);
      
      // 更新 apiKeys 对象，移除失败的 provider
